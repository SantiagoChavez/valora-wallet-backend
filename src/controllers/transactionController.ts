...
const mapTransactionToCamelCase = (tx: Transaction) => ({
    ...
    sourceAmount: tx.source_amount ? Number(parseFloat(tx.source_amount).toFixed(8)) : null,
    targetAmount: tx.target_amount ? Number(parseFloat(tx.target_amount).toFixed(8)) : null,
    exchangeRate: tx.exchange_rate ? Number(parseFloat(tx.exchange_rate).toFixed(8)) : null,
    resultingBalance: tx.resulting_balance ? Number(parseFloat(tx.resulting_balance).toFixed(8)) : null,
